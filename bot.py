...
def otp_result_markup(otp, price=None, full_msg=None):
    # OTP বাটন (সবুজ) + Full Message বাটন (লাল) পাশাপাশি
    row1 = [make_button(otp, style="success", copy_text_val=otp)]
    if full_msg:
        row1.append(make_button("Full Message", style="danger", copy_text_val=full_msg))
    rows = [row1]
    return build_inline_keyboard(rows)
            ...
            try:
                ...
                if panel == "nexus":
                        ...
                        for api_id in api_ids:
                            ...
                            if otp:
                                ...
                                text = (
                                    ...
                                    f"╚━━━━━━━━━━━━━━━╝\n\n"
                                    f"<blockquote>{otp}</blockquote>"
                                )
                                kb = otp_result_markup(otp, price=current_price, full_msg=otp)
                                try:
                    ...
                    if data.get("meta", {}).get("code") == 200:
                        ...
                        kb = otp_result_markup(otp, price=current_price, full_msg=full_msg)
                        try:
                            bot.edit_message_text(text, chat_id, active_msg_id, reply_markup=kb, parse_mode="HTML")
                        except Exception:
                            try:
